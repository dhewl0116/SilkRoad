import { Controller, Post, UseGuards, ValidationPipe, Body, Req, Get} from '@nestjs/common';
...
export class MemoController {
    ...
    @Post('save')
    async save_txt(@Body(ValidationPipe) memo_DTO:memo_dto, @Req() req):Promise<Memo>{
        console.log("save")
    ...
    @Get('recall')
    async recall_txt(@Body(ValidationPipe) memo_get_DTO:memo_get_dto, @Req() req){
        console.log("recall")
